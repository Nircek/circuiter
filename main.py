...
class element:
    ...
    def render(self):
        self.parent.arc(self.p.x + self.s.w // 2, self.p.y + self.s.h //
                        2, min(self.s.w, self.s.h) // 2, 0, 360, self.st)
...
class switch(element):
    ...
    def render(self):
        ...
        if st == 'black' and self.power:
            st = 'red'
        self.parent.arc(self.p.x + self.s.w // 2, self.p.y +
                        self.s.h//2, min(self.s.w, self.s.h)//2, 0, 360, st)
...
class light(element):
    ...
    def render(self):
        ...
        if st == 'black' and self.power:
            st = 'red'
        self.parent.w.create_rectangle(
            self.p.x, self.p.y,
            self.p.x + self.s.w, self.p.y + self.s.h, outline=st)
        if len(self.inputs) >= self.slots:
            pc = self.parent.getPowerColor(self.inputs[0], self.UUID)
            e = self.parent.get(self.inputs[0]).e
            self.parent.w.create_line(e.x, e.y, self.p.x + self.s.w//2,
                                      self.p.y + self.s.h//2, fill=pc)
...
class ANDgate(element):
    ...
    def render(self):
        self.parent.w.create_line(
            self.p.x, self.p.y, self.p.x + 20, self.p.y, fill=self.st)
        self.parent.w.create_line(
            self.p.x, self.p.y, self.p.x, self.p.y + 40, fill=self.st)
        self.parent.w.create_line(self.p.x, self.p.y + 40,
                                  self.p.x + 20, self.p.y + 40, fill=self.st)
        ...
        for i in range(len(self.inputs)):
            ...
            self.parent.w.create_line(
                self.p.x, self.p.y + j, self.p.x - 10, self.p.y + j, fill=pc)
            e = self.parent.get(self.inputs[i]).e
            self.parent.w.create_line(
                e.x, e.y, self.p.x - 10, self.p.y + j, fill=pc)
...
class ORgate(element):
    ...
    def render(self):
        self.parent.arc(self.p.x - 20, self.p.y + 20,
                        math.sqrt(2 * 20**2), 315, 90, self.st)
        self.parent.w.create_line(
            self.p.x, self.p.y, self.p.x + 20, self.p.y, fill=self.st)
        self.parent.w.create_line(self.p.x, self.p.y + 40,
                                  self.p.x + 20, self.p.y + 40, fill=self.st)
        ...
        for i in range(len(self.inputs)):
            ...
            pc = self.parent.getPowerColor(self.inputs[i], self.UUID)
            self.parent.w.create_line(self.p.x + k, self.p.y + j,
                                      self.p.x - 10, self.p.y + j, fill=pc)
            e = self.parent.get(self.inputs[i]).e
            self.parent.w.create_line(
                e.x, e.y, self.p.x - 10, self.p.y + j, fill=pc)
...
class NOTgate(element):
    ...
    def render(self):
        self.parent.w.create_line(
            self.p.x, self.p.y, self.p.x, self.p.y + 40, fill=self.st)
        self.parent.w.create_line(
            self.p.x, self.p.y, self.p.x + 32, self.p.y + 20, fill=self.st)
        self.parent.w.create_line(self.p.x, self.p.y + 40,
                                  self.p.x + 32, self.p.y + 20, fill=self.st)
        self.parent.arc(self.p.x + 36, self.p.y + 20, 4, 0, 360, self.st)
        if len(self.inputs) >= self.slots:
            pc = self.parent.getPowerColor(self.inputs[0], self.UUID)
            self.parent.w.create_line(
                self.p.x, self.p.y + 20, self.p.x - 10, self.p.y + 20, fill=pc)
            e = self.parent.get(self.inputs[0]).e
            self.parent.w.create_line(
                e.x, e.y, self.p.x - 9, self.p.y + 20, fill=pc)
...
class UUIDs:
    ...
    def arc(self, x, y, r, s, e, outline='black'):
        if e >= 360:
            self.w.create_arc(x - r, y - r, x + r, y + r,
                              start=0, extent=180, style=ARC, outline=outline)
            self.w.create_arc(x - r, y - r, x + r, y + r,
                              start=180, extent=180, style=ARC, outline=outline)
        else:
            self.w.create_arc(x - r, y - r, x + r, y + r,
                              start=s, extent=e, style=ARC, outline=outline)
